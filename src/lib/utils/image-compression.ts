...
export async function compressImage(
    ...
    try {
        ...
        console.log(`🔍 Tentando comprimir: "${file.name}" | Tipo: "${file.type}" | Tamanho: ${(file.size / 1024).toFixed(1)} KB`);
        ...
        if (!file.type.startsWith('image/') && !isImageExtension) {
            console.warn(`⚠️ compressImage: O arquivo "${file.name}" não parece ser uma imagem válida. Ignorando compressão.`);
            return file;
        }
        ...
        if (isGenericType && isImageExtension) {
            console.log(`💡 Detectada extensão de imagem com tipo genérico. Forçando conversão para image/${extension === 'png' ? 'png' : 'jpeg'}`);
