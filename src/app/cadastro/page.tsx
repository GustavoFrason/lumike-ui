    ...
    const validateField = (name: string, value: string) => {
        ...
        switch (name) {
            case 'name':
                ...
                else if (value.trim().split(' ').length < 2) errorMessage = 'Digite seu nome completo (Sobrenome).';
                break;
            case 'email':
                if (!value.trim()) errorMessage = 'E-mail é obrigatório.';
                else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) errorMessage = 'Digite um e-mail válido.';
                break;
            case 'whatsapp':
                // Optional field, but if filled, must be valid length
                if (value && value.replace(/\D/g, '').length < 10) errorMessage = 'WhatsApp inválido (mínimo 10 dígitos).';
                break;
            case 'confirmSenha':
                ...
                <CardContent>
                    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                        <div className="space-y-1">
                            ...
                            />
                            {fieldErrors.name && <p className="text-[10px] text-red-500 font-medium pl-1">{fieldErrors.name}</p>}
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            />
                            {fieldErrors.email && <p className="text-[10px] text-red-500 font-medium pl-1">{fieldErrors.email}</p>}
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            />
                            {fieldErrors.whatsapp && <p className="text-[10px] text-red-500 font-medium pl-1">{fieldErrors.whatsapp}</p>}
                        </div>
                        ...
                        <div className="relative space-y-1">
                            ...
                            {showPasswordRules && (
                                <div className="mt-2 p-3 bg-gray-50/50 border border-gray-100 rounded text-xs text-medium-gray space-y-1 transition-all">
                                    ...
                                    <div className={`flex items-center gap-2 ${passwordCriteria.length ? 'text-green-600' : 'text-gray-400'}`}>
                                        <div className={`w-1.5 h-1.5 rounded-full ${passwordCriteria.length ? 'bg-green-600' : 'bg-gray-300'}`} />
                                        Mínimo de 8 caracteres
                                    </div>
                                    <div className={`flex items-center gap-2 ${passwordCriteria.uppercase ? 'text-green-600' : 'text-gray-400'}`}>
                                        <div className={`w-1.5 h-1.5 rounded-full ${passwordCriteria.uppercase ? 'bg-green-600' : 'bg-gray-300'}`} />
                                        Pelo menos 1 letra maiúscula
                                    </div>
                                    <div className={`flex items-center gap-2 ${passwordCriteria.symbol ? 'text-green-600' : 'text-gray-400'}`}>
                                        <div className={`w-1.5 h-1.5 rounded-full ${passwordCriteria.symbol ? 'bg-green-600' : 'bg-gray-300'}`} />
                        ...
                        <div className="space-y-1">
                            ...
                            />
                            {fieldErrors.confirmSenha && <p className="text-[10px] text-red-500 font-medium pl-1">{fieldErrors.confirmSenha}</p>}
                        </div>

                        {error && <p className="text-red-500 text-sm text-center bg-red-50 p-2 rounded">{error}</p>}

                        <Button type="submit" disabled={loading || !isFormValid} className="w-full bg-deep-black hover:bg-primary-gold text-white font-bold uppercase tracking-widest mt-2 h-12 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                            {loading ? (
