...
import { Search, Plus, Trash, Save, ArrowLeft, QrCode, CreditCard, Wallet, User as UserIcon } from 'lucide-react';
...
export default function NovaVendaPage() {
    ...
    async function handleFinalizar() {
        ...
        if ((paymentMethod === 'boca' || paymentMethod === 'aberto' || paymentStatus === 'aberto' || paymentStatus === 'parcial') && !selectedCustomer) {
            setError('⚠️ Vendas fiadas ou em aberto precisam ter um cliente selecionado para rastreamento em "Clientes a Pagar"');
            return;
        }
        ...
        try {
            ...
            const payload: CreateOrderDto = {
                ...
                boca_details: paymentMethod === 'boca' ? {
                    value: parseFloat(bocaValue.replace(/\./g, '').replace(',', '.')) || 0,
                    paid_now: paymentStatus === 'parcial' ? parseFloat(bocaPaidNow.replace(/\./g, '').replace(',', '.')) || 0 : undefined,
                    notes: bocaNotes
                } : undefined,
                card_details: paymentMethod === 'cartao' || paymentMethod === 'parcelado' ? {
                    brand: cardBrand,
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                        ...
                        <div className="flex gap-2">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    autoFocus
                                    className="w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--lumike-gold)] text-lg"
                                />
                            </div>
                            <button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pb-8">
                        {loadingProducts ? (
                            ...
                            filteredProducts.map(product => (
                                <div key={product.id} className="bg-white p-3 rounded-lg border hover:border-[var(--lumike-gold)] transition flex gap-3 group relative cursor-pointer"
                                    onClick={() => addToCart(product)}
                                >
                                    <div className="w-16 h-16 bg-zinc-100 rounded relative overflow-hidden flex-shrink-0">
                                        {product.images && product.images.length > 0 ? (
                                            <Image src={product.images[0].url} alt={product.name} fill className="object-cover" />
                                        ) : (
                                            <div className="w-full h-full flex items-center justify-center text-zinc-300">Sem foto</div>
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-2 text-sm text-zinc-500">
                                            ...
                                            {product.sku2 && <span className="bg-zinc-100 px-1 rounded text-xs">{product.sku2}</span>}
                                        </div>
                                        <p className="text-[var(--lumike-gold)] font-bold mt-1">
                                    ...
                                    <div className="absolute right-3 bottom-3 opacity-0 group-hover:opacity-100 transition">
                                        <Plus className="bg-[var(--lumike-gold)] text-white p-1 h-6 w-6 rounded-full shadow-lg" />
                                    </div>
                                </div>
                            ))
                        ) : (
                ...
                <div className="space-y-4">
                    <div className="bg-white p-6 rounded-lg border border-zinc-200 shadow-lg sticky top-6 max-h-[calc(100vh-2rem)] overflow-y-auto">
                            ...
                            {cart.length === 0 ? (
                                ...
                                cart.map(item => (
                                    <div key={item.product.id} className="flex gap-3 text-sm group">
                                        <div className="w-10 h-10 bg-zinc-100 rounded relative overflow-hidden flex-shrink-0">
                                            {item.product.images?.[0]?.url && <Image src={item.product.images[0].url} alt="" fill className="object-cover" />}
                                        </div>
                                        ...
                                        <div className="flex flex-col items-end gap-1">
                                            ...
                                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="px-1 text-zinc-400 hover:text-zinc-600">-</button>
                                                <span className="text-xs w-4 text-center">{item.quantity}</span>
                                                <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="px-1 text-zinc-400 hover:text-zinc-600">+</button>
                                                <button onClick={() => removeFromCart(item.product.id)} className="ml-1 text-red-400 hover:text-red-600"><Trash className="h-3 w-3" /></button>
                                            </div>
                                        </div>
                                    </div>
                                ))
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        disabled={paymentMethod === 'dinheiro' || paymentMethod === 'cartao' || paymentMethod === 'parcelado'}
                                        className={`w-full border rounded p-2 text-sm font-medium ${paymentMethod === 'dinheiro' || paymentMethod === 'cartao' || paymentMethod === 'parcelado'
                                            ? 'bg-green-50 text-green-700 border-green-200 cursor-not-allowed'
                                            : paymentStatus === 'pago' ? 'bg-green-50 text-green-700 border-green-200' :
                                                paymentStatus === 'parcial' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
                            ...
                            <div className="bg-amber-50 p-3 rounded border border-amber-200 space-y-2 animate-in slide-in-from-top-2">
                                ...
                                {paymentStatus === 'parcial' && (
                                    <div className="space-y-2">
                                        <div>
                                            <label className="text-xs text-amber-700 font-medium block mb-1">💰 Valor Pago Agora</label>
                                            <CurrencyInput
                                                ...
                                                onValueChange={(val) => {
                                                    ...
                                                    const paidAmount = parseFloat((val || '0').replace(/\./g, '').replace(',', '.'));
                                        ...
                                        <div>
                                            <label className="text-xs text-amber-700 font-medium block mb-1">📋 Valor Pendente (Calculado)</label>
                                            <CurrencyInput
                                                value={bocaValue}
                                ...
                                />
                                {selectedCustomer && (
                                    <Link href="/admin/clientes-pagar" target="_blank" className="text-xs text-amber-700 hover:underline flex items-center gap-1">
                        ...
                        <div>
                            <label className="text-xs font-semibold uppercase text-zinc-500 mb-1 block">Observações da Venda</label>
                            <textarea
                                rows={2}
                                value={notes}
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-[var(--lumike-gold)] text-white rounded-lg font-bold text-lg hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-orange-100"
                    >
                        {creating ? <Loading size="sm" spinnerClassName="text-white" /> : (
                            <>
