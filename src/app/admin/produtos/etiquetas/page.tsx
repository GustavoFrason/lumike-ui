                ...
                <div className="bg-white p-4 rounded-lg border border-zinc-200 md:col-span-1 h-[calc(100vh-280px)] flex flex-col">
                    <div className="mb-4 space-y-2">
                        ...
                        <div className="flex gap-2">
                            <button onClick={selectAllFiltered} className="text-xs text-[var(--lumike-gold)] hover:underline">Selecionar Tudo</button>
                            <button onClick={deselectAll} className="text-xs text-red-500 hover:underline">Limpar Seleção</button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 pr-2">
                        {filteredProducts.map(product => (
                            <div
                                ...
                                className={`p-3 rounded border cursor-pointer hover:bg-zinc-50 transition ${selectedProducts.find(p => p.id === product.id)
...
function LabelContent({ product, config }: { product: Product, config: { fontSize: number, qrSize: number } }) {
    return (
        <div className="w-full h-full flex items-center gap-1">
            <div className="flex-shrink-0">
                <QRCodeSVG
                    value={product.sku || product.id.toString()}
                    size={config.qrSize}
                    level="L"
                />
            </div>
