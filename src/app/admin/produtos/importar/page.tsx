    ...
    <section className="space-y-6">
      ...
      {step === 1 && (
        <div className="max-w-xl mx-auto mt-12">
          <label className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-zinc-300 rounded-lg cursor-pointer bg-white hover:bg-zinc-50 transition">
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="text-xs text-zinc-500">
                Planilha (XLSX) ou Nota Fiscal Eletrônica (XML)
              </p>
            </div>
            <input
              type="file"
              ...
              <tbody className="divide-y divide-zinc-100">
                {items.map((item, idx) => (
                  <tr
                    key={idx}
                    className={
                      item.action === 'UPDATE_STOCK' ? 'bg-emerald-50/30' : 'bg-blue-50/30'
                    }
                  >
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        {item.existing_product && (
                          <span className="text-xs text-zinc-400">
                            Atual: {item.existing_product.name}
                          </span>
                        )}
                      </div>
                    </td>
