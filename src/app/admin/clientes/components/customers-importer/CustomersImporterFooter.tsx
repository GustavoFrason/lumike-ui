...
interface CustomersImporterFooterProps {
  itemCount: number;
  confirming: boolean;
  onClear: () => void;
  onConfirm: () => void;
...
export function CustomersImporterFooter({
  itemCount,
  confirming,
  onClear,
  onConfirm,
  onCancel,
}: CustomersImporterFooterProps) {
    ...
    <div className="p-6 border-t border-zinc-100 bg-zinc-50 flex gap-3 justify-end items-center">
      ...
      >
        {confirming ? (
          ...
          <>
            Importar {itemCount} {itemCount === 1 ? 'cliente' : 'clientes'}
