...
// Mock data for now
const COUPONS = [
    { code: 'BEMVINDO10', discount: '10%', description: 'Desconto de Boas-vindas', valid_until: 'Indeterminado', status: 'active' },
];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {COUPONS.map((coupon) => (
                    <Card key={coupon.code} className="bg-white border border-light-gray overflow-hidden relative">
                        ...
                        <div className="p-6 flex flex-col items-center text-center">
                            ...
                            <h3 className="font-playfair text-2xl font-bold text-deep-black">{coupon.discount} OFF</h3>
                            ...
                            <div className="mt-6 w-full bg-off-white border-2 border-dashed border-gray-300 p-3 rounded flex items-center justify-between gap-4">
                                <span className="font-mono font-bold text-lg text-deep-black tracking-widest">{coupon.code}</span>
                                <button
                                    onClick={() => copyToClipboard(coupon.code)}
                                    className="text-primary-gold hover:text-deep-black transition-colors"
                                    title="Copiar código"
                                >
                                    {copied === coupon.code ? <Check className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                                </button>
                            </div>
                        </div>
                    </Card>
                ...
                <Card className="bg-gray-50 border border-dashed border-gray-300 p-6 flex flex-col items-center justify-center text-center opacity-70">
                    ...
                    <p className="text-xs text-medium-gray mt-1">Fique atento ao seu WhatsApp para novas ofertas!</p>
                </Card>
            </div>
        </div>
    );
